...
def sift_process(image,sigma,assumed_blur,num_intervals,image_border_width,queue):
    ...
    keypoints, descriptors = get_sift_keypoints_and_descriptors(gray_image,sigma,assumed_blur,num_intervals,image_border_width)
...
def find_scale_space_extrema(gaussian_images, dog_images, num_intervals=3, sigma=1.6, image_border_width=1, contrast_threshold=0.04):
            ...
            for i in range(image_border_width, first_image.shape[0] - image_border_width):
                for j in range(image_border_width, first_image.shape[1] - image_border_width):
                    if is_pixel_an_extremum(first_image[i-1:i+2, j-1:j+2], second_image[i-1:i+2, j-1:j+2], third_image[i-1:i+2, j-1:j+2], threshold):
                        localization_result = localize_extremum_via_quadratic_fit(i, j, image_index + 1, octave_index, num_intervals, dog_images_in_octave, sigma, contrast_threshold, image_border_width)
...
def localize_extremum_via_quadratic_fit(i, j, image_index, octave_index, num_intervals, dog_images_in_octave, sigma, contrast_threshold, image_border_width, eigenvalue_ratio=10, num_attempts_until_convergence=5):
    ...
    for attempt_index in range(num_attempts_until_convergence):
        ...
        gradient = compute_gradient_at_centerpixel(pixel_cube)
...
def compute_gradient_at_centerpixel(pixel_array):
